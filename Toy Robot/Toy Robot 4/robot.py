...
#list of valid gui modules
valid_module_list = ['text','turtle']

#code to dynamically allocate an import path
if len(argv) > 1 and argv[1] in valid_module_list:
    module = importlib.import_module(f"world.{argv[1]}.world")
else:
    module = importlib.import_module("world.text.world")
...
def do_forward(robot_name, steps):
    ...
    carry_on, reason = module.update_position(steps, direction)
    if carry_on == True :
        return True, ' > '+robot_name+' moved forward by '+str(steps)+' steps.'
    elif carry_on == False and reason == "boundary":
...
def do_replay(robot_name, arguments):
    ...
    for (command_name, command_arg) in commands_to_replay:
        ...
        if not silent:
            print(command_output)
            direction = directions[current_direction_index]
            module.show_position(robot_name,direction)
...
def robot_start():
    ...
    ob = []
    ob = world.obstacles.get_obstacles()
